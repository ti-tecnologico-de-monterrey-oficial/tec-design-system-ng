...
| --- | --- | --- |
| Navigation collections | `title-content`, `sidebar` | `bottom-navigation-bar`, `drawer-overlay`, `navigation-bar`, `web-templates` connected 2026-08-13 as composition facades / a documented Storybook fixture — see `INVENTORY.md`. `title-content` stays contract-required: its runtime throws unless `title`/`componentTitle` is set, and it composes `Breadcrumb` + `UserImage`/`BotIcon`/`BoxIcon` + icon, but no plausible stable Bamboo node was found (closest candidate, `Simple header`, only exposes a title text + trailing-icon boolean — it lacks breadcrumb/avatar, so it isn't a truthful match). `sidebar` (`BmbSidebarComponent`) also stays contract-required — documented in `CONTRACT_STATE.md` since the original 2026-08-12 NAV-01 discovery but never folded into this tracker until 2026-09-02: its only structural input is `elements: SidebarElement[][]`, a fully data-driven array, while the published `Sidebar` component (`299:51502`) exposes only an `Expanded` variant and no `Items` SLOT. |
| Projected collections | `action-menu`, `list-group`, `list-items` | `list-group-item` connected 2026-08-27 — see `INVENTORY.md`; its parent `list-group` has no content properties of its own, so it stays contract-required. `action-menu` (`Type` variant is single-row options, not a repeated SLOT) and `list-items` (no independent Bamboo node found) re-checked 2026-08-27, still need a genuine `Items`/`Content` SLOT with published semantic item children. `card-button` connected (partial) 2026-09-02 — its Figma node still only exposes `Size`, so badge/menu/textLink remain unrepresented, but a real per-variant example is now published; see `INVENTORY.md`. |
