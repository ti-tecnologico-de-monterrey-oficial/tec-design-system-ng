...
- Rama local y remota: `code-connect-test`.
- Último commit antes de este handoff: `32e5deb8a` (`docs: add MiTec contract design reference`).
- 68 templates Angular publicados y verificados en la librería Bamboo; 3 adaptadores internos `BB_1_4*` adicionales.
...
- El working tree del repositorio tiene muchísimos cambios ajenos. Sólo se pueden editar, stagear, commitear y subir archivos dentro de `code-connect-batch/`.
...
2. Clase/directiva exportada desde `projects/ds-ng/src/public-api.ts` y su selector/input/proyección público.
...
No mostrar ni leer el valor del token. Cargarlo desde el `.env` de la raíz del repo. No usar el binario `figma` ambiguo del proyecto ni `npm exec … -- figma`.
...
git push origin code-connect-test
```
...
> Continúa el backlog de contratos Bamboo Code Connect en `/Users/csolares/Documents/5 - CODEX/tec.design/Codebase/tec-design-system-ng-official`. Lee por completo `code-connect-batch/HANDOFF.md`, `CONTRACT_IMPLEMENTATION_BACKLOG.md`, `CONTRACT_STATE.md`, `CODE_CONNECT_CONVENTION.md`, `INVENTORY.md`, `REMAINING_COMPONENTS.md`, `CONTRACT_BACKLOG.md`, `DECISIONS.md`, `BB_ADAPTERS.md` y `COMPOSITION_REMEDIATION.md`. Usa MiTec `Jf8Nd71tihhPZdv9xm6PnN` sólo como evidencia de uso y Bamboo `Q4t8qIM5fklC9I3Atc1BrZ` como único destino de contratos/Code Connect. Ejecuta sólo la Fase 0 de `NAV-01` o, si ya está completa, una familia validada; no publiques nada sin nodo Bamboo estable + API Angular pública + Storybook + parse. Usa exclusivamente el CLI oficial señalado en HANDOFF, sin revelar token, sin `--force`, y verifica `hasTemplate:true` con Figma MCP. Edita, commitea y sube exclusivamente `code-connect-batch` a `code-connect-test`.
