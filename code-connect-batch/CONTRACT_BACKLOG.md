...
| Forms and editors: `input-tags`, `text-editor`, `login`, `login-onboarding` | Label, placeholder, helper/error text, required/disabled booleans, and true field/action SLOT where multiple inputs/actions are projected | Individual field can connect first; screen/container stays contract-required until slots exist. |
