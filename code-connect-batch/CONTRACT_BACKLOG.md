...
| Data cards and rubrics: `account-statement`, `balance`-like profile/card APIs, `evaluation-rubric`, `student-activity-card`, `sounds-card`, `digital-id`, `user-profile`, `user-summary-content` | Named text/media fields plus a child SLOT for every repeated action/data row; for rubrics, semantic criterion item, summary label, comment prompt and button labels; do not expose one opaque JSON text field | Design exposes content; Engineering supplies a documented neutral Storybook fixture where required. |
