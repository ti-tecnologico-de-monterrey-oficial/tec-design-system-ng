...
*
docs: add Figma contract implementation backlog
*
chore: connect phone number
*
chore: connect dropdown top bar and user image
*
chore: connect filter and hito components
*
chore: connect user summary
*
chore: connect chat apps and timestream
*
chore: connect container paginator and header
*
chore: connect home and advertisement cards
*
chore: connect balance overview
*
docs: audit retroactive BB compositions
*
chore: connect iframe notification and sidebar
*
...
Trae los 21 templates que solo existían en code-connect-v1.6.4-b (ActionMenu, ActionMenuItemAdapter, Timestream, HomeCardChat, SidebarMobile, LoadingScreen, Skeleton×6, GenericCard, ListItemWithActions, CardButton×3, ChatBarContainer, HitoStatusBadge, HomeCardMobile, ImageCarousel). Resuelve 2 colisiones de nodo: elimina AiChatCard.figma.ts (apuntaba a BmbAIChatCardComponent, componente inexistente) y Loader.figma.ts (duplicado de LoadingScreen). Reescribe las rutas source de la convención old/ a la estructura plana actual. 125 templates, 0 rutas rotas, publicado con CLI 2.0.0.
- **Add structured JSON index of Code Connect inventory (component-index.json)**:
Add structured JSON index of Code Connect inventory (component-index.json)
...
El commit anterior eliminó AiChatCard.figma.ts basándose en una
verificación contra el árbol del 17-ago, donde BmbAIChatCardComponent
no existía. Sí existe: creado el 26-ago en eb3c7c5c8 y exportado en
ui-angular/src/index.ts.

El nodo 9268-46409 "AI Chat Card" corresponde a bmb-ai-chat-card, no a
bmb-home-card-chat — ese era el mapeo de v1.6.4-b, congelada nueve días
antes de que el componente existiera.
...
Compared full ui-angular/src/index.ts (147 exports) against the tracker's
128-export baseline: 16 exports were added since and never triaged.

- bmb-ai-chat-card: connected (composition facade, AI Chat Card 9268:46409)
- bmb-notification-counter: blocked, node only exists in Documentation library
- bmb-accordion-simple-text: blocked, its exact Figma variant shares a
top-level node with the already-connected bmb-accordion; Code Connect
only supports one mapping per top-level node
- other 13 (item/interactive-item variants, layout directives, selector,
accordion-control) triaged by disposition in REMAINING_COMPONENTS.md,
not yet attempted

98/128-baseline templates published. Nothing was unpushed before this.
- **code-connect: triage remaining 13 audit exports (all Blocked or Parent/child)**:
code-connect: triage remaining 13 audit exports (all Blocked or Parent/child)

- 4 bmb-item-* + 3 bmb-interactive-item-* variants: no independent Bamboo
node found (only unrelated matches / internal BB_* building blocks not
meant for standalone use); interactive-item-* also marked for
decommissioning in source. Blocked.
- bmb-layout-grid/-item, bmb-vertical-layout/-item: confirmed pure
structural directives, no visual identity. Blocked.
- bmb-selector: state/class-binding directive, not visual. Blocked.
- bmb-accordion-control: manages the already-connected bmb-accordion's
children. Parent/child composition.
...
Resolved with Carlos's TEC.mobi/Gestor de Rúbricas references:
- navigation-bar (4070:156220) - composition facade
- bottom-navigation-bar (11836:53649) - documented Storybook fixture
- drawer-overlay / FABOverlayDrawer (474:98365) - composition facade
- web-templates / Template_2Column_NormalScreenLEFT (523:207773) - composition facade
...
header-mobile (required text + documented Storybook fixture) and
mobile-templates (2 of 8 template values: calendar, external-link — the
other six are ambiguous among near-identical TemplateMobile_* nodes, left
unconnected rather than guessed). All verified hasTemplate:true via Figma MCP.
Updates INVENTORY.md, REMAINING_COMPONENTS.md, CONTRACT_BACKLOG.md, DECISIONS.md.
- **feat(component) remover .event**:
feat(component) remover .event
- **docs: connect Table and Server table via documented Storybook fixtures**:
...
Resolved the ambiguity among ~15 near-duplicate Template_Table_* Bamboo
variants by cross-checking a real "Programa académico" table instance
(badge status column, remove_red_eye edit-action icon, sticky columns,
bottom paginator) against bmb-table-lite's config surface. Matches
Template_Table_EditActions_Badge_Web (9252:163784), a sibling of the
already-connected server-table variant. hasTemplate:true verified via
Figma MCP.
- **docs+feat: connect login-onboarding using Carlos's TEC.mobi reference**:
docs+feat: connect login-onboarding using Carlos's TEC.mobi reference

Resolved the ambiguity between three same-named "Login_boardingintro"
Bamboo components by cross-checking BmbLoginOnboardingService's default
activePage=0 state (bmb-login-onboarding-login, zero inputs) against the
