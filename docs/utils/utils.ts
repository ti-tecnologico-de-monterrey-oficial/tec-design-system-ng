...
import { getListingOnOneLine } from '../../ui-angular/src/lib/_shared/logic/utils';
...
const TECHNICAL_DOC_TITLE = `${DESIGN_SYSTEM_TITLE} ***- Technical documentation***`;
const TECHNICAL_DOC_REFERENCES = `Please remember to refer to the ${TECHNICAL_DOC_TITLE} for more details:`;
...
export const FULLSCREEN_DESC = `${RELEVANT_TITLE.note}
...
const getKeyFormat = (key: string, value: unknown): string =>
  (typeof value === 'function' && `(${key})`) || `[${key}]`;

export const attributes = (object: Record<string, unknown>): string =>
...
export const attributesText = (object: Record<string, unknown>): string =>
  Object.entries(object)
    .filter(([key]) => key === 'test_text')
    .map(([, value]) => `${value}`)
    .join(' ');
...
export const getDeprecatedDesc = ({
  ...
  const _content = `This ${type} will not be maintainable.
...
export const getElementUsesDesc = (name: string, isOther = true) =>
  getAlertBlockquote(
...
export const getVariableDetail = (
  ...
  const _description = `Content with ${list}${isInherit ? ' class names' : ''} applied using CSS variables.`;
  return `
...
export const getSandboxConsiderationsDocumentation = (
  ...
  style = '',
  isClassNameVar = false,
  isInherit = false,
  isOmitImportant = false,
): string => {
  let _implementationDetails = '';

  if (implementationDetails.length) {
    const splitChar = '|||';
    const patternToReplaceOneLine = /(and )|(, )/g;
    ...
    const variableStyles = `${style.concat(
      style ? ' ' : '',
      getMergeList(
        varList,
