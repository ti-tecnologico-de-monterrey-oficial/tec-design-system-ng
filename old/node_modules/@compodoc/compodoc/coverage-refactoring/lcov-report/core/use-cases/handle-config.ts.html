...
<head>
    <title>Code coverage report for core/use-cases/handle-config.ts</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../index.html">All files</a> / <a href="index.html">core/use-cases</a> handle-config.ts</h1>
        <div class='clearfix'>
...
&nbsp;
    public static getInstance() {
        <span class="missing-if-branch" title="else path not taken" >E</span>if (!HandleConfigFile.instance) {
...
&nbsp;
    public async handle(currentProgram: CLIProgram) {
        return new Promise(async (resolve, reject) =&gt; {
            try {
                if (<span class="missing-if-branch" title="if path not taken" >I</span>currentProgram.config) {
                    let configFilePath = currentProgram.config;
                    this.configFilePath = configFilePath;
                    let testConfigFilePath = configFilePath.match(process.cwd());
                    <span class="missing-if-branch" title="if path not taken" >I</span>if (testConfigFilePath &amp;&amp; <span class="branch-1 cbranch-no" title="branch not covered" >testConfigFilePath.length &gt; 0)</span> {
<span class="cstat-no" title="statement not covered" >                        configFilePath = configFilePath.replace(process.cwd() + path.sep, '');</span>
                    }
&nbsp;
...
<span class="cstat-no" title="statement not covered" ><span class="cstat-no" title="statement not covered" >                    this.c</span>onfigExplorerResult = <span class="cstat-no" title="statement not covered" >await this.configExplorer.search();</span></span>
<span class="cstat-no" title="statement not covered" >                    if (this.configExplorerResult) {</span>
<span class="cstat-no" title="statement not covered" >                        this.configFilePath = path.basename(this.configExplorerResult.filepath);</span>
                    }
                }
