...
} from '../../components/old/bmb-timestream/types';

const checkFilters = (
  event: ITimelineEvent,
  filters: IBmbTimestreamFilters,
) => {
  ...
  const result = filtersKeys.map((filter) => {
    switch (filter) {
      ...
      case 'text':
        ...
        const substr = new RegExp(
          `(^.*[${allowedSeparator}]${filters[filter]}$)|(^${filters[filter]}[${allowedSeparator}].*)|(^${filters[filter]}$)|(^.*[${allowedSeparator}]${filters[filter]}[${allowedSeparator}].*$)`,
        );
        return (
          substr.test(event.title) ||
